...
글은 꾸준히 쌓였는데, 정작 그 글을 *읽고 쓰는* 화면은 손이 덜 간 자리가 많았다. 그래서 날을 잡고 네 부분으로 나눠서 다듬게 되었다. 읽는 화면, 쓰는 화면, 글 안의 다이어그램/표, [그래프](/graph/)다. 무엇을 더했는지, 그게 왜 필요했는지, 만든 코드 한두 줄을 같이 첨부해서 적어두려고 한다.
