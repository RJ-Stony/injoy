...
강조한 한 줄이 판단의 전부다. 제목들을 위에서부터 차례로 보다가, 화면 위쪽 기준 높이(`LINE`, 88px)를 넘어 올라간 마지막 제목에서 멈춘다. 그게 지금 보고 있는 단락이다. 스크롤 이벤트는 `requestAnimationFrame`으로 묶어, **한 프레임에 한 번만 다시 계산**하게 했다. 매 스크롤마다 위치를 계산하면 무겁기 때문이다.
