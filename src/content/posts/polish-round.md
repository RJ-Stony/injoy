---
title: "그래프가 멈추질 않았다: 조용한 무한 루프 잡기"
description: "배치가 끝내 안 멈춰 매 프레임 다시 그리던 버그를 찾아낸 이야기"
pubDate: 2026-06-18
...
강조한 한 줄이 판단의 전부다. 제목들을 위에서부터 차례로 보다가, 화면 위쪽 기준 높이(`LINE`, 88px)를 넘어 올라간 마지막 제목에서 멈춘다. 그게 지금 보고 있는 단락이다. 스크롤 이벤트는 `requestAnimationFrame`으로 묶어, 한 프레임에 한 번만 다시 계산하게 했다. 매 스크롤마다 위치를 계산하면 무겁기 때문이다.
...
가장 흥미로운 건 [그래프](/graph/)였다. 휴대폰에서 미세하게 렉이 걸려서 들여다봤더니, 점들을 펼쳐 놓는 시뮬레이션이 영영 멈추질 않고 있었다. 점끼리 서로 밀어내고 끌어당기는 힘이 어중간하게 맞물려, 자리가 미세하게 계속 떨리며 배치가 끝내 가라앉질 않았다. 그 탓에 화면을 쉼 없이 다시 그리고 있었다. 눈에 잘 안 띄는, 조용한 버그였다.
