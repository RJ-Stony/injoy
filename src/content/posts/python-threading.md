...
[[singleton-pattern]] 글에서 캐시 매니저를 만들 때 `threading.Lock`으로 스레드 안전성을 지켰다. 그때 잠깐 스친 **threading 모듈**을 이번엔 제대로 파 보자.

파이썬의 스레드 역사는 주요 언어 중에서도 독특하다. threading 모듈은 1999년에 나온 1.5.2 버전부터 있었다. 그런데 유명한 단서가 하나 붙는다. 바로 **GIL**(Global Interpreter Lock; 한 번에 한 스레드만 파이썬 바이트코드를 실행하게 하는 전역 잠금)이다. GIL은 파이썬 객체 접근을 보호하는 **뮤텍스**(mutex; 한 번에 하나만 지나가게 하는 잠금)라, 여러 스레드가 동시에 파이썬 바이트코드를 실행하지 못하게 막는다.
...
이 글에서는 `threading.Thread` API, 스레드를 만드는 여러 방법(함수 참조, 상속, 호출 가능 객체), 설정, 생명주기 관리, 실무에서 만나는 패턴까지 하나씩 짚어 볼 예정이다.

## 1. Thread 클래스

파이썬 스레드 모델의 중심에는 `threading.Thread`가 있다. 프로그램의 모든 스레드는 이 클래스, 또는 그 하위 클래스의 인스턴스로 표현된다.
...
# 전체 생성자 시그니처
threading.Thread(
    group=None,      # 향후 확장용 예약 (항상 None)
    target=None,     # 스레드에서 실행할 콜러블
...
스레드는 동작과 디버깅에 영향을 주는 속성 몇 가지를 노출한다.
...
**ident** - 스레드가 시작될 때 배정되는 고유 정수 식별자다. OS가 준 식별자이고, 시작하지 않은 스레드에서는 None이다.
...
**native_id** - 커널이 배정하는 네이티브 스레드 ID다(파이썬 3.8+). `ident`와 달리 `top`이나 `ps` 같은 시스템 도구에서 보이는 값과 일치한다.
...
**is_alive()** - 스레드가 시작됐고 아직 안 끝났으면 True를 돌려준다.
...
파이썬에서 스레드는 그저 어떤 콜러블을 실행하는 "일꾼"이다. threading 모듈은 그 일꾼이 무엇을 실행할지 정하는 깔끔한 방법 세 가지를 준다.
...
세 방법 모두 결국 같은 일을 한다. 스레드가 실행할 콜러블을 넘겨준다. 차이는 **작업과 상태를 어떻게 포장하느냐**다.
