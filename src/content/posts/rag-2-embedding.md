...
pubDate: 2026-07-12
updatedDate: 2026-07-14T00:14:44.024Z
category: "AI"
...
사내 문서를 AI가 대신 찾아 주게 만들자는 계획을 세우고 리서치를 마치고 나니, 결정할 게 딱 두 개 남았다. 무엇으로 **문장을 숫자로 바꿀 것**인가, 그리고 **그 숫자를 어디에 저장할 것인가**였다.
...
필수 요건은 두 가지였다. 하나, 데이터를 사내에 보관할 수 있을 것. 인사/재무 문서를 다루니 데이터를 외부로 흘려보내면 안 된다. 둘, 한국어 검색이 정확할 것. 사내 문서가 거의 다 한국어라, 영어 위주 모델은 아무리 유명해도 뒤로 밀린다.
...
- **한국어 검색 성능 1순위.** 79.44로 한국어 특화 오픈소스 모델 중 최상위다. bge-m3는 8192 토큰까지 긴 글을 통째로 넣을 수 있다는 강점이 있었지만, 한국어 검색 점수에서 4점 넘게 뒤처졌다.
...
arctic-embed 시리즈는 MTEB Retrieval 리더보드에서 **모델 크기 대비 성능이 좋은 모델**이라, 작은 서버로도 최상위권 검색 품질을 낼 수 있다는 점이 컸다. 대신 못 박아 두기 애매한 부분은 재평가 시점으로 미뤘다. 6개월이 지나거나, 새 한국어 특화 오픈소스 모델이 나오거나, 한국어 검색 정확도에 사용자 불만이 쌓이면 다시 보려고 했다.
...
물론 공짜는 아니다. HNSW 인덱스 튜닝을 직접 배워야 했고, 백업과 모니터링 절차도 손수 구축해야 하며, 장애가 나면 사내 책임으로 돌아간다. 이 값을 치르고도 호환성/메타데이터/비용의 강점이 압도적이라 종합 1위를 줬다.

## 테이블과 인덱스

스키마는 관련 도서를 참고해 패턴을 거의 그대로 가져왔다. 이 테이블 하나가 문서 조각(청크)과 그 벡터, 필터에 쓸 메타데이터를 한 곳에 담는다.
...
검색 과정은 함수 하나로 감쌌다. 이 함수가 결국 하는 일은, 질문 벡터와 가장 가까운 문서 청크를 부서의 필터까지 걸어서 뽑아 오는 것이다.
...
강조한 부분이 유사도를 계산하는 절차다. `<=>`는 코사인 거리라, `1 -` 을 붙여 거리를 닮은 정도로 뒤집는다. 바로 아래 `WHERE`의 `filter_department`가 앞서 정한 기준 네 번째(메타데이터 필터링)를 그대로 만들어준다. 벡터로 비슷한 걸 찾으면서, 볼 수 있는 권한이 있는 부서의 문서만 걸러 낸다.
...
위에서 강조한 부분이 **오래된 백업을 알아서 지우는 부분**이다. 백업만 쌓고 지우지 않으면 디스크가 언젠가 차게 된다. 이 밖에 주 1회 `VACUUM ANALYZE`로 통계 데이터를 갱신하고, 인덱스가 비대해지면 분기에 한 번 `REINDEX`로 다시 만든다. 지표는 `pg_stat`이 Grafana로 들어가고, 임계치를 넘으면 공지 채널로 알림이 가게 한다.
