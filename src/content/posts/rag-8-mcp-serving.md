---
title: "검색 하나로 회사 기억에 접속하기"
description: "MCP 서버 하나로 문서·대화·정형 데이터·작업 기록을 묶어 서빙한 마무리"
...
MCP 서버는 AI에게 "이런 도구들을 쓸 수 있다"고 알려 주는 창구다. 사내 하네스나 각자 PC의 Claude Desktop 같은 AI가 이 서버에 연결하면, 서버가 내건 도구를 골라 직접 호출하고 그 결과로 답을 만든다. 우리가 몇 주에 걸쳐 지은 파이프라인은 전부 이 서버 뒤에 숨고, 쓰는 사람은 그냥 자연어로 묻기만 하면 된다.

## AI가 부르는 도구들, 검색과 SQL을 가른다

이 서버에 도구를 여럿 달았다. 성격은 크게 넷으로 갈린다.

- **문서 검색.** Notion에서 온 청크를 의미 기반으로 뒤진다. "인수인계 회의록 있어?"처럼 주제나 키워드로 묻는 질문이 여기로 간다.

- **대화 검색.** Slack 스레드를 같은 방식으로 뒤진다. "결제 관련 논의 어디까지 됐어?" 같은, 문서엔 없고 대화에만 남은 답을 찾는다.

- **SQL 분석.** 고객 유입, 거래, 웹 트래픽처럼 숫자로 딱 떨어지는 사내 정형 데이터를 SQL로 집계한다. "이번 달 신규 유입 몇 건이야?", "진행 중인 건 합계 얼마야?"가 이쪽이다.

- **메모리 저장.** AI가 작업하다 얻은 요약이나 해결법을 장기 기억에 적어 둔다. 다음 검색에 그대로 반영된다.

여기서 정작 중요한 건 도구를 나눈 기준이다. **텍스트로 찾을 일과 숫자로 셀 일을 갈랐다.** 주제나 키워드로 훑는 건 검색으로, 집계·카운팅·날짜 범위 필터처럼 검색으로는 어려운 정량 분석은 SQL로 보낸다. 날짜만 걸린 질문("오늘 올라온 문서 뭐야?")은 최근 페이지를 훑는 전용 도구를 따로 두어, 의미 검색을 헛돌리지 않게 했다.

이 구분을 굳이 못박은 데는 사연이 있다. [[rag-6-slack-pipeline|앞서]] 겪은 대로, AI는 텍스트를 찾으라고 해도 종종 SQL 도구로 `ILIKE` 문자열 매칭을 돌리려 든다. 그게 더 직관적으로 보이는 모양이다(?). 도구를 잘 만드는 것과, AI가 그 도구를 제대로 고르게 만드는 건 별개의 일이었다.

## 도구 설명이 곧 사용 설명서

AI가 도구를 고르는 근거는 딱 하나, 도구에 붙은 **설명**이다. 사람이 API 문서를 읽듯, AI는 이 설명만 보고 "이 질문엔 이 도구"라고 판단한다. 그래서 설명을 라벨이 아니라 사용 설명서처럼 썼다.
...
이 설명 한 덩어리에 [[rag-5-notion-pipeline|파이프라인]]을 지으며 배운 함정이 다 들어 있다. 결과마다 원본 링크가 붙으니 인용하라는 것, 필터로 범위를 좁힐 수 있다는 것, 한 페이지가 여러 청크로 쪼개져 있어 첫 결과만 믿으면 안 된다는 것. 무엇보다 **표 같은 구조적 내용은 청크에 안 담긴다**는 한계를 적고, 그럴 땐 SQL로 그 페이지의 청크 전체를 읽으라는 우회로까지 일러 준다. 검색이 못 잡는 걸 SQL이 메우게, 두 도구가 서로를 가리키게 엮은 것이다.

Slack 검색 설명도 같은 결이다. 스레드가 여러 청크로 나뉘어 한 조각만으로는 대화 전체가 안 보인다는 것, 참여자 목록은 스레드 단위 정보라 그 조각에 없는 사람도 섞여 있을 수 있다는 것을 적어 뒀다. AI가 헷갈릴 지점을 미리 짚어 주는 주석에 가깝다.
...
두 번째는 검색 서비스를 하나로 유지하는 것이다. 테이블이 둘로 늘었다고 검색 서비스까지 둘로 두면, 부를 때마다 양쪽을 각각 뒤져 결과를 합쳐야 한다. 대신 기존 **Cortex Search**(Snowflake가 임베딩·색인·검색을 대신 맡는 관리형 검색) 서비스의 소스를 두 테이블의 합집합으로 한 번만 바꿨다.
...
강조한 `UNION ALL` 한 줄이 두 창고를 한 검색에 묶는다. 검색 서비스는 그대로 하나라, 작업 기록이 붙었다고 서빙 비용이 더 들지 않는다. 소스를 한 번 바꾸면서 기존 청크를 다시 색인하는 1회성 비용만 감수하면 됐다.
...
강조한 두 줄이 권한 경계의 전부다. `EXECUTE AS OWNER`라, 호출하는 직원이 아니라 프로시저를 만든 관리자의 권한으로 INSERT가 돈다. 직원 계정 자체에는 그 테이블에 쓸 권한이 없어도, 이 프로시저를 통해서만 딱 한 테이블에 행을 보탤 수 있다. 그마저도 프로시저 안은 정적 INSERT라 값을 바꿔 넣을 뿐, 임의의 SQL을 끼워 넣을 여지가 없다. 카테고리는 코드가 `HARNESS_MEMORY`로 강제로 박아, 직원이 다른 카테고리인 척 위장할 수도 없다.
