---
title: "다이어그램과 코드에 재생 버튼을 달았다"
description: "시퀀스 다이어그램/코드 블록 스텝 재생과 읽기 모션을 더한 과정"
pubDate: 2026-07-06
category: "블로그"
tags: ["ui", "다이어그램", "글쓰기"]
draft: false
...
모바일로 [[singleton-pattern]] 글을 읽다가 이상한 걸 발견했다. 시퀀스 다이어그램의 노트 박스 안 텍스트가 **수직 중앙이 아니라 위로 살짝 떠** 있었다. 박스는 제자리인데 글자만 천장에 붙어 있는 모양새다. 이걸 고치러 들어갔다가, 결국 다이어그램에 재생 버튼을 달고, 코드 블록에 실행 순서를 입히고, 읽는 화면 곳곳에 잔모션을 더하는 큰 회차가 되었다. 이번 글에서는 그 과정을 하나씩 짚어 볼 예정이다.

## 박스 위로 떠 있던 텍스트

먼저 버그부터. 원인은 바로 전날 넣은 옵션이었다. 긴 한글 노트가 박스 밖으로 새는 문제를 잡으려고 mermaid에 `sequence: { wrap: true }`를 켰는데, 이 wrap 모드가 **텍스트의 세로 배치를 바꾼다**. 데스크톱 Chrome에서는 멀쩡한데 iOS Safari에서는 글자가 박스 위로 떠 보이게 된다. 브라우저마다 SVG 텍스트의 기준선(baseline; 글자가 앉는 가상의 줄)을 다르게 처리하기 때문이다.

브라우저마다 다르게 어긋나는 문제라면, 어긋난 원인을 좇기보다 **어긋난 결과를 실측해서 되돌리는 쪽**이 확실하다. 그래서 렌더가 끝난 직후에 박스와 텍스트의 실제 좌표를 재서, 차이만큼 텍스트를 옮기는 보정을 렌더러에 더했다.
...
결국 이 코드는 박스 중앙과 텍스트 중앙의 차이를 두 축으로 재서, 그만큼 텍스트를 밀어 넣는 일을 한다. 1픽셀 미만이면 손대지 않으니 **이미 정렬된 브라우저에서는 아무 일도 일어나지 않는다.** 어긋난 브라우저에서만 조용히 발동하는 안전망인 셈이다.

여기서 한 가지 배운 게 있다. 처음 계획은 노트의 **줄마다** 따로 중앙 보정을 하는 것이었는데, 실측해 보니 그건 함정이었다. 두 줄짜리 노트의 각 줄을 전부 박스 중앙으로 옮기면 두 줄이 같은 자리에 겹치게 된다. 그래서 여러 줄을 한 덩어리로 보고 **덩어리 전체를 같은 거리만큼** 옮기는 방식으로 바꿨다. 계획서의 코드를 그대로 옮겨 적지 않고 실제 화면을 재 본 덕에 잡은 문제다.

> [!NOTE]
> **공용 렌더러의 덤**
> \
> 이 보정은 글 본문, 확대 모달, 글쓰기 화면의 미리보기가 공유하는 렌더러에 들어갔다. 한 곳을 고치면 세 화면이 같이 고쳐지고, 앞으로 새로 그리는 시퀀스 다이어그램에도 자동으로 적용된다.

## 시퀀스 다이어그램에 재생 버튼

버그를 고치고 나니 욕심이 생겼다. 시퀀스 다이어그램은 원래 **시간 순서**가 있는 그림이다. 위에서 아래로 메시지가 오가는 순서가 곧 이야기의 순서인데, 완성본을 한 번에 보여주면 그 순서가 눈에 잘 안 들어온다. 다른 사이트에서 단계별로 하나씩 나타나는 시퀀스 다이어그램을 봤던 기억이 나서, 우리 것에도 재생 버튼을 달기로 했다.
...
구현에서 가장 재미있던 부분은 **스텝을 어떻게 나누느냐**였다. mermaid가 그린 SVG에는 "이건 1번째 메시지"라는 표시가 없다. 처음에는 세로 좌표가 비슷한 요소끼리 묶으면 되겠지 하고 12픽셀 밴드로 그룹핑을 설계했는데, 실측하니 메시지 텍스트가 자기 화살표보다 35픽셀이나 위에 있었다. 밴드로는 못 묶는다. 그래서 규칙을 바꿨다. 요소를 세로 순서로 줄 세운 뒤, **메시지 텍스트가 나타날 때마다 새 스텝을 연다.** 그 사이의 화살표, 노트, 활성화 박스는 직전 스텝에 따라붙는다.
...
강조한 한 줄이 그룹핑의 전부다. 메시지 텍스트가 스텝의 문을 열고, 나머지는 열린 문으로 들어간다. 확대 모달에서도 같은 플레이어가 붙는데, 스텝 수집이 화면 좌표 기반이라 **줌 변환이 걸리기 전에** 붙여야 한다는 순서 함정이 하나 있었다.
