...
pubDate: 2026-07-12
updatedDate: 2026-07-19T13:57:15.674Z
category: "AI"
...
자체 호스팅을 설계했던 과정에서, 서버부터 데이터의 권한 관리까지 전부 내가 담당하려던 부분은, 결국 관리형 서비스 쪽으로 방향을 틀게 되었다. 이미 사내에 사용하던 Snowflake가 있었고, 거기에 임베딩과 검색 서비스도 제공하고 있으니 서버를 따로 만들고 새벽에 백업 과정까지 책임질 이유가 줄어들었다. 방향을 정했으니 이제 필요한 데이터를 적재할 차례였다. 첫번째 소스는 텍스트 위주의 사내 자료가 가장 두껍게 쌓인 **Notion**이었다.

그런데 막상 들어오는 Raw 테이블을 열어 보니 문제가 있었다. Notion 데이터는 우리가 읽는 "**페이지**"의 형태로 들어오지 않는다. 문단 하나, 제목 하나, 체크박스 하나가 전부 따로 떨어진 **블록**(block; Notion이 글을 쪼개서 저장하는 최소 단위)으로 쌓이고 있었다. 그래서 한 페이지를 되살리려면 흩어진 블록들을 다시 이어 붙이는 과정이 필요했다.
...
이 원천 데이터는 **Airbyte**로 매일 한 번씩 Snowflake의 특정 스키마에 적재되고 있었다. 해당 스키마를 열어 보니 블록이 약 14만 행, 페이지가 약 7천 행, 데이터베이스가 216개였다. 반정형 JSON 그대로 적재되었기 때문에, 검색에 사용하려면 이것을 AI가 편히 읽을 수 있는 페이지 단위로 되돌리는 일부터 해야 했다.
...
가장 까다로운 건 블록이 *어느 페이지에 속하는지* 알아내는 일이었다. **인라인 데이터베이스 안에 있는 블록**은, 부모가 페이지가 아니라 또 다른 블록의 형태였다. 그래서 부모를 타고 위로 계속 올라가야 **페이지**가 나온다. 그래서 부모 관계를 재귀적으로 탐색하는 매핑 테이블을 만들었다.
...
위에서 강조한 부분이 재귀 과정의 전부이다. 부모가 아직 블록이면 그 부모의 부모를 다시 탐색해 한 단계 또 올라가고, 부모가 페이지가 되는 순간 멈추게끔 했다. 무한히 이 재귀가 돌아가는 것을 막기 위해 **최대 10단계까지만** 파고들게 해뒀다. (실제 데이터로 측정해보니 현재까지는 6단계가 최고였다.)
...
블록을 페이지로 되돌리고 카테고리와 작성자를 붙이는 이 과정을, 매번 사람이 돌릴 수는 없었다. 원본이 바뀔 때마다 알아서 다시 계산되어야 했다. 그래서 **Dynamic Table**(정의한 SELECT 결과를 실제 테이블로 유지하고, 정한 주기마다 스스로 다시 계산해 채우는 Snowflake의 기능)을 네 단계로 엮었다.
...
마지막 단계에서 페이지 전문을 잘랐다. **청킹**(chunking; 긴 글을 검색하기 좋게 작은 조각으로 나누는 과정)은 500자로 자르되 다음 조각이 앞 조각 끝에서 50자를 겹쳐 시작하게 했다. 경계에 걸친 문장이 둘로 쪼개져 검색에서 통째로 놓이는 일을 막으려는 것이다.
...
청크가 준비됐으니 실제로 검색이 되게 해야 했다. 캐스케이드의 마지막 결과를 물리 테이블 하나(`NOTION_CHUNKS`)로 옮겨 담고, 그 위에 **Cortex Search**(Snowflake가 제공하는 검색 서비스; 벡터 유사도와 키워드 검색 방식을 함께 쓰는 하이브리드 검색)를 얹었다. 약 9천 개의 청크가 올라갔다. 이 서비스는 소스 테이블에 변경 추적을 걸어 두면 청크가 바뀔 때 알아서 다시 색인하고, 검색 결과를 의미 유사도로 한 번 더 재정렬해 준다. 임베딩 모델은 한국어를 지원하면서 비용이 덜 드는 쪽으로 골랐다.
