...
[[rag-3-selfhosted|자체 호스팅 설계]]에서 서버부터 권한까지 전부 우리가 지려던 그림은, 결국 관리형 쪽으로 방향을 틀게 되었다. 이미 사내에 데이터 창고로 쓰던 Snowflake가 있었고, 거기에 임베딩과 검색이 딸려 있으니 서버를 새로 세우고 새벽 백업까지 떠안을 이유가 줄었다. 방향을 정했으니 이제 진짜 데이터를 올릴 차례였다. 첫 소스는 사내 지식이 가장 두껍게 쌓인 Notion으로 골랐다.

그런데 막상 원천 테이블을 열어 보니 문제가 거기서 시작됐다. Notion 데이터는 우리가 읽는 "문서" 모양으로 들어오지 않는다. 문단 하나, 제목 하나, 체크박스 하나가 전부 따로 떨어진 **블록**(block; Notion이 글을 쪼개 저장하는 최소 단위)으로 온다. 한 페이지를 되살리려면 흩어진 블록을 다시 이어 붙여야 했다.

## Notion은 문서가 아니라 블록으로 온다

Notion의 구조는 Workspace 아래로 페이지가 있고, 페이지 안에 블록이 있고, 블록 안에 또 블록이 재귀적으로 중첩된다. 인라인 데이터베이스나 하위 페이지가 얼마든지 겹겹이 들어갈 수 있다.

이 원천 데이터는 Airbyte로 매일 한 번 Snowflake의 원본 스키마에 적재되고 있었다. 열어 보니 블록이 약 13.6만 행, 페이지가 약 6.7천 행, 데이터베이스가 216개였다. 반정형 JSON 그대로라, 검색에 쓰려면 이걸 사람이 읽는 페이지 단위로 되돌리는 일부터 해야 했다.

```mermaid
flowchart TB
    subgraph src["원천 (Airbyte 일배치)"]
        B["BLOCKS 약 13.6만"]
        P["PAGES 약 6.7천"]
        D["DATABASES 216"]
    end
    ...
    subgraph dt["Dynamic Table 캐스케이드"]
...
왼쪽 원천에서 오른쪽 검색 서비스까지, 블록이 조각조각 흘러 들어와 페이지로 뭉쳐지고 다시 검색용 청크로 잘려 나가는 전체 그림이다. 하나씩 뜯어보자면.

## 블록을 페이지로 되돌리기

가장 까다로운 건 블록이 *어느 페이지에 속하는지* 알아내는 일이었다. 인라인 데이터베이스 안의 블록은 부모가 페이지가 아니라 또 다른 블록이라, 부모를 타고 위로 계속 올라가야 페이지가 나온다. 그래서 부모 관계를 재귀로 훑는 매핑을 만들었다.
...
강조한 줄이 재귀의 전부다. 부모가 아직 블록이면 그 부모의 부모를 다시 붙여 한 단계 더 올라가고, 부모가 페이지가 되는 순간 멈춘다. 무한히 도는 걸 막으려고 최대 10단계까지만 파고들게 막아 뒀다.

여기서 끝이 아니었다. 페이지를 찾았어도 그 페이지가 *어느 카테고리에 속하는지*는 또 위로 올라가 봐야 알 수 있다. 페이지가 페이지에 속하고, 그 페이지가 데이터베이스에 속하고, 그게 다시 상위 페이지에 속하는 사슬이 얽혀 있어서다. 그래서 페이지에서 최상위 루트까지 6단계를 반복 조인으로 추적하는 매핑을 하나 더 뒀다. 최상위에는 열두 개의 루트 페이지와 네 개의 루트 데이터베이스가 있었고, 여기까지 올라가야 비로소 "이 문서는 어느 부서 것"인지가 정해졌다.

작성자도 비슷한 문제였다. 원천에는 사람 이름이 아니라 내부 ID만 있어서, ID를 실제 구성원 이름으로 바꾸는 표를 손으로 만들어야 했다. 시스템이 채워 주는 사용자 테이블이 비어 있던 탓이다. 등재한 25건 중에도 사람 17명, 봇 2개까지는 정리됐지만 정체를 못 밝힌 6건은 숙제로 남았다.

## 스스로 갱신되는 Dynamic Table 캐스케이드
...
강조한 **TARGET_LAG**(원본이 바뀐 뒤 늦어도 이만큼 안에는 반영한다는 갱신 지연 목표)이 이 테이블의 성격을 정한다. 하루로 잡아 두면 원본이 어제 바뀐 내용이 오늘 안에는 검색에 반영된다. 블록을 뽑는 첫 단계만 바뀐 블록만 다시 처리하는 증분 모드로 둬서 비용을 아꼈고, 나머지는 통째로 다시 계산하게 했다.
