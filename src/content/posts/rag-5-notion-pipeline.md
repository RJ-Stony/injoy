...
자체 호스팅 설계에서 서버부터 권한까지 전부 내가 지려던 그림은, 결국 관리형 쪽으로 방향을 틀게 되었다. 이미 사내에 데이터 창고로 쓰던 Snowflake가 있었고, 거기에 임베딩과 검색이 딸려 있으니 서버를 새로 세우고 새벽 백업까지 떠안을 이유가 줄었다. 방향을 정했으니 이제 진짜 데이터를 올릴 차례였다. 첫 소스는 사내 지식이 가장 두껍게 쌓인 Notion으로 골랐다.
