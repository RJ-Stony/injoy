...
description: "YAGNI 원칙이 무엇인지, 미리 만든 코드가 왜 문제가 되는지 정리하기"
pubDate: 2026-07-07
...
말로는 당연해 보인다. 필요 없는 걸 왜 만드나 싶다. 그런데 막상 코드를 짜다 보면 이 원칙을 어기는 순간이 자주 온다. 플러그인이 하나뿐인데 **플러그인 시스템**을 만들고, 하나만 구현했는데도 팩토리 클래스를 두고, 아무도 바꾸지 않을 설정의 옵션을 더한다. 전부 "나중에 필요할지도 모르니까"라는 말로 스스로를 설득하면서 만드는 구조이다.
...
이 원칙은 초기 애자일 방법론인 **익스트림 프로그래밍**(Extreme Programming; 짧은 주기로 자주 배포하며 요구사항 변화에 맞춰 가는 개발 방식)에서 나왔다. 소프트웨어 요구사항은 끊임없이 변하니, 예상한 미래를 위해 미리 만드는 일은 낭비라는 생각에서부터 나온 것이다. **지금 당장 동작하는 가장 단순한 것**을 내놓고, 거기서부터 조금씩 고쳐 나가는 게 좋다는 태도다.
...
- **불어나는 복잡도** - 유연함이 늘게 되면 움직여야 하는 부품도 늘게 된다. 이해하기 어렵고, 테스트와 수정도 그만큼 힘들어진다. 새로 합류한 동료가 `NotificationEngine`과 팩토리 클래스, 인터페이스를 보면 "다 이유가 있어서 이렇게 짰겠지" 하고 지레짐작한다. 함부로 단순하게 못 바꾼다. 지어낸 코드가 얼떨결에 영구히 남는다.
...
- **미리 정해진 아키텍처 제약** - 가용성 SLA가 계약에 걸려 있거나, 시작부터 지역 간의 복제를 해야 한다고 이미 아는 경우엔 일부 구조를 일찍 정해야 한다. 처음부터 고가용성을 염두에 두지 않은 시스템에 나중에 끼워 넣는 비용이, 처음부터 넣는 비용보다 훨씬 크기 때문이다.
