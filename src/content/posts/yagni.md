...
이 원칙은 초기 애자일 방법론인 **익스트림 프로그래밍**(Extreme Programming; 짧은 주기로 자주 배포하며 요구사항 변화에 맞춰 가는 개발 방식)에서 나왔다. 소프트웨어 요구사항은 끊임없이 변하니, 예상한 미래를 위해 미리 만드는 일은 낭비라는 생각에서부터 나온 것이다. **지금 당장 동작하는 가장 단순한 것**을 내놓고, 거기서부터 조금씩 고쳐 나가는 게 좋다는 것이다.
