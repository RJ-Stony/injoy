...
pubDate: 2026-07-09
updatedDate: 2026-07-09T23:31:44.052Z
category: "개발"
tags: ["네트워크", "NAT", "포트포워딩"]
draft: false
series: "네트워크"
...
집에 있는 PC에 웹 서버 하나를 띄웠다고 하자. 같은 집 안에서는 브라우저에 `192.168.0.10:8080`만 치면 잘 열린다. 그런데 기분 전환하려고 바깥에 있는 카페에 가서 노트북으로 그 서버에 접속하려고 하면 아무리 해도 연결되지 않는다. 분명 서버는 켜져 있는데 말이다.

여기서 **포트포워딩**(port forwarding)이 등장한다. 밖에서 우리 집 안의 특정 PC로 들어오는 길을 뚫어 주는 기술이다. 그런데 이걸 제대로 이해하려면, "**왜 원래는 밖에서 못 들어오는가**"부터 알아야 한다. 그 배경에는 **NAT**라는 게 있고, 이 NAT에는 "원래는 주소가 부족했다"라는 역사가 숨어있다.
...
- **사설 IP**(private IP) - 우리 집이나 회사 **안에서만** 연결되는 주소(`192.168.x.x`, `10.x.x.x` 같은 것). 집집마다 똑같은 주소를 써도 상관없다. 어차피 그 집 안에서만 쓰기 때문이다.

여기서 이 글 내내 쓸 비유를 하나 세우려고 한다.

> 우리 집(또는 회사) 네트워크는 **건물** **하나**다.
>
...
건물에는 **1층 안내데스크**가 있다. 드나드는 모든 우편물이 이 데스크를 반드시 거친다. 우리 집으로 치면 **공유기**(라우터, 게이트웨이)가 바로 이 안내데스크다. 그리고 안내데스크가 하는 주된 일이 **NAT**(Network Address Translation; 네트워크 주소 변환), 즉 **주소 바꿔치기**다.

202호 사람이 바깥 회사(예: 어떤 웹사이트)로 택배를 보낸다고 하자. 택배에는 원래 이렇게 적혀 있다.

- 받는 사람: 바깥 회사 주소

- 보내는 사람: **202호** (사설 IP)

이 정보릉 그대로 내보내면 큰일이다. 받는 회사가 답장을 쓰려고 보내는 사람 칸을 보면 "202호"라고만 적혀 있는데, 바깥 우체국은 그 202호가 세상 어느 건물의 202호인지 알 방법이 없다. **그래서 답장을 못 보낸다.**

그리하여 안내데스크에서 택배가 **밖으로 나가는 순간** 주소를 고쳐 쓴다. 보내는 사람 칸을 **202호에서 우리 건물 대표 주소(공인 IP)로** 바꾼다. 그리고 장부에 한 줄 적어 둔다. "*방금 나간 이 택배의 원래 주인은 202호다.*"
...
그럼 카페에서 우리 집의 서버로 접속이 안 되던 이유는 뭘까??

지금까지는 전부 **안에서 먼저 말을 건 경우**였다. 202호에서 택배를 보낸 사실을 안내데스크가 알고 장부에 적어 뒀기 때문에 답장을 받아올 수 있었다. 그런데 이번엔 반대다. 바깥의 낯선 손님(카페의 내 노트북)이 **먼저** 우리 집으로 택배를 보낸다.

- 받는 사람: 우리 건물의 대표 주소

- 보내는 사람: 카페의 노트북

이 택배가 안내데스크에 도착한다. 안내데스크는 난감하다. 대표 주소로 오긴 왔는데, **이 손님을 몇 호로 보내야 할지** 알 수가 없다. 장부에는 이 택배에 대한 정보가 없다. (안에서 먼저 시작한 적이 없기 때문) 건물에 컴퓨터가 여러 대인데, 이 택배가 201호 것인지 202호 것인지 안내데스크로선 알 도리가 없다. 그래서 그냥 **문전박대**하게 된다.
