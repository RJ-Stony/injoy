...
> `src/content/posts/`에 마크다운 파일을 하나 추가하면, 바로 발행된다.

추가 설정을 건드릴 필요도, 어디에 등록할 필요도 없다. 이 글은 그 과정을 단계별로
...
푸시가 곧 배포다. RSS(`/rss.xml`)와 사이트맵도 빌드할 때마다 자동으로 갱신되니
따로 신경 쓸 것이 없다.

---

정리하면 이렇다. **파일 하나, frontmatter 일곱 줄, 그리고 푸시.** 글쓰기를 가로막는
