...
그 설계를 접게 만든 건 새 논문도, 리뷰 지적도 아니었다. 데이터 웨어하우스 인수인계 자리에서 지나가듯 들은 한마디였다.

> "회사가 이미 연 단위로 계약해 둔 Snowflake가 있어요. 크레딧이 거의 안 쓰이고 놀고 있고, 거기에 임베딩 기능도 있는 것 같던데요."

세 문장이었는데, 다 지어 놓은 설계가 통째로 흔들렸다. 있는 것 같다(?)는 말 한마디를 확인하러 자리로 돌아갔다.
...
그 안에 **Cortex Search**(임베딩·색인·하이브리드 검색·리랭킹을 SQL만으로 대신 해 주는 Snowflake의 관리형 검색 서비스)가 있었다. 문서를 넣어 두면 임베딩을 알아서 만들고, 벡터 색인을 세우고, 검색과 리랭킹까지 붙여 준다. 내가 [[rag-3-selfhosted|자체 호스팅으로 직접 세우려던 것]]을, 이미 계약해 둔 창고가 SQL 몇 줄로 하고 있었다.
...
강조한 줄이 핵심이다. 임베딩 모델 이름 하나만 적으면, 문장을 벡터로 바꾸는 일을 Snowflake가 관리형으로 맡는다. `TARGET_LAG`은 원본이 바뀌면 얼마나 빨리 색인이 따라잡을지를 정하는 값이라, 새벽마다 도는 인덱싱 잡을 내가 짤 필요가 없어진다. [[rag-3-selfhosted|직접 세우려던]] 임베딩 서버, HNSW 인덱스 튜닝, cron 잡이 이 SQL 한 덩어리로 접힌다.
...
관리형으로 넘어가며 가장 오래 붙든 건 권한이었다. 자체 호스팅에선 [[rag-3-selfhosted|검색 함수 안에 부서·등급 필터를 박아]] 권한 밖 문서를 아예 후보에서 빼는 방식으로 풀었다. 관리형에선 그 필터를 내가 못 짜니, Snowflake가 주는 권한 장치에 기대야 했다.
...
대신 얻은 게 컸다. 서버, 임베딩 파이프라인, 벡터 DB 운영, 새벽 백업, 모니터링이 통째로 사라졌다. [[rag-3-selfhosted|"전부 우리가 든다"]]던 그림에서 사람 손이 메우기로 했던 자리가 거의 다 관리형으로 넘어갔다. 무엇보다 이미 계약해 둔 크레딧 안에서 도니, **추가 인프라 비용이 0**이었다. 놀고 있던 창고를 쓰는 것뿐이라.
